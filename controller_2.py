...
class Tutorial (object):
  ...
  def _handle_PacketIn (self, event):
    ...
    if dpid not in self.gateways:
        self.gateways[dpid]=[IPAddr('10.0.0.1'), IPAddr('20.0.0.1'), IPAddr('30.0.0.1')]
    if dpid not in self.arpTable:
        self.arpTable[dpid]={}
        self.arpTable[dpid][IPAddr('10.0.0.1')] = EthAddr("02:00:de:ad:be:11")
        self.arpTable[dpid][IPAddr('20.0.0.1')] = EthAddr("02:00:de:ad:be:12")
        self.arpTable[dpid][IPAddr('30.0.0.1')] = EthAddr("02:00:de:ad:be:13")
    if dpid not in self.routeTable:
        self.routeTable[dpid]={}
        self.routeTable[dpid][IPAddr('10.0.0.2')] = RouteEntry(1, IPAddr('10.0.0.2'))
        self.routeTable[dpid][IPAddr('20.0.0.2')] = RouteEntry(2, IPAddr('20.0.0.2'))
        self.routeTable[dpid][IPAddr('30.0.0.2')] = RouteEntry(3, IPAddr('30.0.0.2'))
    if dpid not in self.arpWait:
        self.arpWait[dpid]={}
    if dpid not in self.port_to_ip:
        self.port_to_ip[dpid]={}
        self.port_to_ip[dpid][1]=IPAddr('10.0.0.1')
        self.port_to_ip[dpid][2]=IPAddr('20.0.0.1')
        self.port_to_ip[dpid][3]=IPAddr('30.0.0.1')
    log.debug('DPID %d dealing with packets' % (dpid))
    router_handler(self, dpid, packet, packet_in)    #only router in scenario2
